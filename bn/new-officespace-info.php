...
$_SESSION['comebackUrl']['new-officespace-info'] = $pathUrl;

if(!isset($_SESSION['id'])){
	
	header('Location:login?er=login-need&cbp=new-officespace-info');
	exit;
...
<head>
    <meta charset="utf-8">
    <title>কী খুঁজি - অফিস-এর তথ্য</title>
...
$urlBackPage = 'mark?content=officespace';
...
if(isset($_SESSION['new-officespace-info-error'][$rand])){
	...
	$floor = $_SESSION['new-officespace-info-error'][$rand]['floorBengali'];
	...
	$total_floors = $_SESSION['new-officespace-info-error'][$rand]['total_floorsBengali'];
...
<body onload="startTime()">
	<div class="topnav">
        ...
        <div class="topnav-right">
            ...
            <a href="logout?cbp=new-officespace-info">লগ আউট</a>
        </div>
    </div>
    ...
    <div id="nonFooter">
        <form action="" method="post">
            <table id="officespaceInfo">
                <tr id="heading"><td colspan="2">অফিসটি সম্পর্কে কিছু তথ্য লিখুন</td></tr>
                <tr>
                ...
                <tr>
                    <td><span id="securityMoneyName" <?php if($sale == 1){ ?>style="display:none"<?php } ?> >জমা মূল্য<input type="button" id="button_scmoney" value="?"><span id="sidenote_scmoney">অফিসটি ব্যবহার শুরুর আগে যে পরিমাণ টাকা দিতে হবে, যা অফিসটি ছাড়ার পূর্ব পর্যন্ত জমা থাকবে ।</span></span><span id="bookingMoneyName" <?php if($rent == 1){ ?>style="display:none"<?php } ?> >বুকিং মূল্য<input type="button" id="button_scmoney" value="?"><span id="sidenote_scmoney">কেনার প্রক্রিয়া শুরু করার জন্য শুরুতেই যে পরিমাণ অর্থ দিতে হবে ।</span></span></td> <td><input type="text" name="security-money" <?php if(isset($security_money)){ ?>value="<?php echo $security_money; ?>"<?php } ?> pattern="^[0-9.]+"> টাকা <input type="checkbox" name="security-money-checkbox" id="security-money-checkbox" <?php if($security_money_nego == 1){ echo 'checked'; } ?>> আলোচনাসাপেক্ষ <span id="sc" class="sidenote">একটি সঠিক পরিমাণ উল্লেখ করুন</span></td>
                </tr>
                <tr>
                    <td>পুরো ঠিকানা</td> <td><textarea name="full-address" placeholder="এই অংশটি বড় করে আপনার বিজ্ঞাপনে দেখানো হবে । এখানে আপনার ঠিকানা ব্যাতীত অন্য কিছু লিখবেন না । যেমনঃ দালান নং : ৩০, রোড নং : ২/ক, বাংলা রোড, ঢাকা, বাংলাদেশ । অন্য যেকোনো অতিরিক্ত তথ্য 'অন্যান্য বর্ণনা' ঘরটিতে লিখতে পারেন - যেটি এই ফর্মটির নিচেই খুঁজে পাবেন ।"><?php if(isset($full_address)){ echo $full_address; } ?></textarea> <span id="address" class="sidenote">যথাযথ চিহ্ন বা সংখ্যা ব্যবহার করুন</span></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td>অফিস নম্বর</td> <td><input type="text" name="office-no" <?php if(isset($flat_no)){ ?>value="<?php echo $flat_no; ?>"<?php } ?> > <span id="flat" class="sidenote">সঠিক চিহ্ন বা সংখ্যা ব্যবহার করুন</span></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td>অফিসটির আকার</td> <td><input type="text" name="office-size" <?php if(isset($size_of_flat)){ ?>value="<?php echo $size_of_flat; ?>"<?php } ?> pattern="^[0-9.]+"> বর্গফুট <span id="fl-sz" class="sidenote">একটি সঠিক আকার লিখুন</span></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td>অন্যান্য বর্ণনা</td> <td><textarea name="other-description" placeholder="যেকোনো অতিরিক্ত তথ্য এখানে লিখতে পারেন । যেমনঃ জায়গাটি আলো-বাতাস সমৃদ্ধ । এখানে ৩ টি ঘর এবং ২টি বাথরুম আছে........, ইত্যাদি । অতিরিক্ত ফোন নম্বরঃ 01..........., 02........... অতিরিক্ত ইমেইলঃ no-reply@kikhuji.com"><?php if(isset($other_description)){ echo $other_description; } ?></textarea> <span id="oth-des" class="sidenote">সঠিক চিহ্ন বা সংখ্যা ব্যবহার করুন</span></td>
                </tr>
                <tr>
                	<td>ছবি</td>
                    <td id="photos">
...
if(isset($_POST['submit'])){
	...
	$_SESSION['new-officespace-info-error'][$rand]['floorBengali'] = $floor;
	...
	if($floor != ''){
		...
		if(!preg_match("/^[0-9-]+$/",$floor)){
			...
			$_SESSION['new-officespace-info-error'][$rand]['floor'] = $floor - 1;
	...
	$_SESSION['new-officespace-info-error'][$rand]['total_floorsBengali'] = $total_floors;
	...
	if($total_floors != ''){
		...
		if(!preg_match("/^[0-9-]+$/",$total_floors)){
			...
			$_SESSION['new-officespace-info-error'][$rand]['total_floors'] = $total_floors - 1;
	...
	$_SESSION['new-officespace-info'][$rand]['floor'] = $_SESSION['new-officespace-info-error'][$rand]['floor'];
	$_SESSION['new-officespace-info'][$rand]['floorBengali'] = $floor;
	...
	$_SESSION['new-officespace-info'][$rand]['total_floors'] = $_SESSION['new-officespace-info-error'][$rand]['total_floors'];
	$_SESSION['new-officespace-info'][$rand]['total_floorsBengali'] = $total_floors;

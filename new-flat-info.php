...
<head>
    <meta charset="utf-8">
    <title>Ki Khuji - New Flat Info</title>
...
if(isset($_SESSION['new-flat-info-error'][$rand])){
	...
	$floor = $_SESSION['new-flat-info-error'][$rand]['floor'];
	...
	$total_floors = $_SESSION['new-flat-info-error'][$rand]['total_floors'];
...
<body onload="startTime()">
	<div class="topnav">
        ...
        <div class="topnav-right">
            ...
            <a href="logout?cbp=new-flat-info">Log Out</a>
        </div>
    </div>
    ...
    <div id="nonFooter">
        <form action="" method="post">
            <table id="flatInfo">
                <tr id="heading"><td colspan="2">Enter some information about the Flat</td></tr>
                <tr>
                ...
                <tr id="maxNumPeople" <?php if($mess == 0){ ?>style="display:none"<?php } ?> >
                    <td>Maximum number of people (in case of using as a mess)<input type="button" id="button_maxppl" value="?"><span id="sidenote_maxppl">If a group of single people stay in the flat, maximum number of people who can stay.</span></td> <td><input type="text" name="max-people-in-mess" <?php if(isset($max_people)){ ?>value="<?php echo $max_people; ?>"<?php } ?> pattern="^[0-9]+"> <span id="mx-ppl" class="sidenote">Use a valid number</span></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td><span id="securityMoneyName" <?php if($sale == 1){ ?>style="display:none"<?php } ?> >Security money<input type="button" id="button_scmoney" value="?"><span id="sidenote_scmoney">Amount that will be taken before moving to the flat and won't be returned before leaving the flat.</span></span><span id="bookingMoneyName" <?php if($rent == 1){ ?>style="display:none"<?php } ?> >Booking money<input type="button" id="button_scmoney" value="?"><span id="sidenote_scmoney">Amount that will be taken initially to start purchasing procedures.</span></span></td> <td><input type="text" name="security-money" <?php if(isset($security_money)){ ?>value="<?php echo $security_money; ?>"<?php } ?> pattern="^[0-9.]+"> BDT <input type="checkbox" name="security-money-checkbox" id="security-money-checkbox" <?php if($security_money_nego == 1){ echo 'checked'; } ?>> Negotiable <span id="sc" class="sidenote">Enter a valid amount</span></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td>Flat no.</td> <td><input type="text" name="flat-no" <?php if(isset($flat_no)){ ?>value="<?php echo $flat_no; ?>"<?php } ?> > <span id="flat" class="sidenote">Use valid characters</span></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td>Number of balconies</td> <td><input type="text" name="number-of-balconies" <?php if(isset($number_of_balconies)){ ?>value="<?php echo $number_of_balconies; ?>"<?php } ?> pattern="^[0-9]+"> <span id="num-bal" class="sidenote">Enter a valid number</span></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td>Other description</td> <td><textarea name="other-description" placeholder="Write any additional information you want to mention. e.g. The flat has a beautiful balcony on the south side. One of the washrooms contains commode facility and a bathtub........, etc. Additional phone number : 01............, 02........... Additional Email : no-reply@kikhuji.com"><?php if(isset($other_description)){ echo $other_description; } ?></textarea> <span id="oth-des" class="sidenote">Use valid characters</span></td>
                </tr>
                <tr>
                	<td>Photos</td>
                    <td id="photos">
...
if(isset($_POST['submit'])){
	...
	if($floor != ''){
		...
		if(!preg_match("/^[0-9-]+$/",$floor)){
			...
			$_SESSION['new-flat-info-error'][$rand]['floorBengali'] = $floor + 1;
	...
	if($total_floors != ''){
		...
		if(!preg_match("/^[0-9-]+$/",$total_floors)){
			...
			$_SESSION['new-flat-info-error'][$rand]['total_floorsBengali'] = $total_floors + 1;
	...
	$_SESSION['new-flat-info'][$rand]['floor'] = $floor;
	$_SESSION['new-flat-info'][$rand]['floorBengali'] = $_SESSION['new-flat-info-error'][$rand]['floorBengali'];
	...
	$_SESSION['new-flat-info'][$rand]['total_floors'] = $total_floors;
	$_SESSION['new-flat-info'][$rand]['total_floorsBengali'] = $total_floors;
	$_SESSION['new-flat-info'][$rand]['total_floorsBengali'] = $_SESSION['new-flat-info-error'][$rand]['total_floorsBengali'];

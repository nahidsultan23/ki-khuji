...
<head>
    <meta charset="utf-8">
    <title>Ki Khuji - New Mess Info</title>
...
if(isset($_SESSION['new-mess-info-error'][$rand])){
	...
	$floor = $_SESSION['new-mess-info-error'][$rand]['floor'];
	...
	$total_floors = $_SESSION['new-mess-info-error'][$rand]['total_floors'];
...
<body onload="startTime()">
	<div class="topnav">
        ...
        <div class="topnav-right">
            ...
            <a href="logout?cbp=new-mess-info">Log Out</a>
        </div>
    </div>
    ...
    <div id="nonFooter">
        <form action="" method="post">
            <table id="messInfo">
                <tr id="heading"><td colspan="2">Enter some information about the Mess</td></tr>
                <tr>
                ...
                <tr id="maxNumPeople">
                    <td>Maximum number of people in the room<input type="button" id="button_maxppl" value="?"><span id="sidenote_maxppl">Maximum number of people who will share the same room containing this mess space.</span></td></td> <td><input type="text" name="max-people-in-mess" <?php if(isset($max_people)){ ?>value="<?php echo $max_people; ?>"<?php } ?> pattern="^[0-9]+"> <span id="mx-ppl" class="sidenote">Use a valid number</span></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td>Security money<input type="button" id="button_scmoney" value="?"><span id="sidenote_scmoney">Amount that will be taken before the person moves to the mess and won't be returned until the space is left.</span></td> <td><input type="text" name="security-money" <?php if(isset($security_money)){ ?>value="<?php echo $security_money; ?>"<?php } ?> pattern="^[0-9.]+"> BDT <input type="checkbox" name="security-money-checkbox" id="security-money-checkbox" <?php if($security_money_nego == 1){ echo 'checked'; } ?>> Negotiable <span id="sc" class="sidenote">Enter a valid amount</span></td>
                </tr>
                <tr>
...
if(isset($_POST['submit'])){
	...
	if($floor != ''){
		...
		if(!preg_match("/^[0-9-]+$/",$floor)){
			...
			$_SESSION['new-mess-info-error'][$rand]['floorBengali'] = $floor + 1;
	...
	if($total_floors != ''){
		...
		if(!preg_match("/^[0-9-]+$/",$total_floors)){
			...
			$_SESSION['new-mess-info-error'][$rand]['total_floorsBengali'] = $total_floors + 1;
	...
	$_SESSION['new-mess-info'][$rand]['floor'] = $floor;
	$_SESSION['new-mess-info'][$rand]['floorBengali'] = $_SESSION['new-mess-info-error'][$rand]['floorBengali'];
	...
	$_SESSION['new-mess-info'][$rand]['total_floors'] = $total_floors;
	$_SESSION['new-mess-info'][$rand]['total_floorsBengali'] = $_SESSION['new-mess-info-error'][$rand]['total_floorsBengali'];

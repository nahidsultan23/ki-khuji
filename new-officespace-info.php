...
<body onload="startTime()">
	<div class="topnav">
        ...
        <div class="topnav-right">
            ...
            <a href="logout?cbp=new-officespace-info">Log Out</a>
        </div>
    </div>
    ...
    <div id="nonFooter">
        <form action="" method="post">
            <table id="officespaceInfo">
                ...
                <tr>
                    <td><span id="securityMoneyName" <?php if($sale == 1){ ?>style="display:none"<?php } ?> >Security money<input type="button" id="button_scmoney" value="?"><span id="sidenote_scmoney">Amount that will be taken before the person start using the office space and won't be returned until the office is left.</span></span><span id="bookingMoneyName" <?php if($rent == 1){ ?>style="display:none"<?php } ?> >Booking money<input type="button" id="button_scmoney" value="?"><span id="sidenote_scmoney">Amount that will be taken initially to start the purchasing process.</span></span></td> <td><input type="text" name="security-money" <?php if(isset($security_money)){ ?>value="<?php echo $security_money; ?>"<?php } ?> pattern="^[0-9.]+"> BDT <input type="checkbox" name="security-money-checkbox" id="security-money-checkbox" <?php if($security_money_nego == 1){ echo 'checked'; } ?>>Negotiable <span id="sc" class="sidenote">Enter a valid amount</span></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td>Other description</td> <td><textarea name="other-description" placeholder="Write any additional information you want to mention. e.g. The place is spacious and well ventilated. There are 3 rooms and 2 washrooms........, etc. Additional phone number : 01............, 02........... Additional Email : no-reply@kikhuji.com"><?php if(isset($other_description)){ echo $other_description; } ?></textarea> <span id="oth-des" class="sidenote">Use valid characters</span></td>
                </tr>
                <tr>
                	<td>Photos</td>
                    <td id="photos">
...
if(isset($_POST['submit'])){
	...
	if($floor != ''){
		...
		if(!preg_match("/^[0-9-]+$/",$floor)){
			...
			$_SESSION['new-officespace-info-error'][$rand]['floorBengali'] = $floor + 1;
	...
	if($total_floors != ''){
		...
		if(!preg_match("/^[0-9-]+$/",$total_floors)){
			...
			$_SESSION['new-officespace-info-error'][$rand]['total_floorsBengali'] = $total_floors + 1;
	...
	$_SESSION['new-officespace-info'][$rand]['floorBengali'] = $_SESSION['new-officespace-info-error'][$rand]['floorBengali'];
	...
	$_SESSION['new-officespace-info'][$rand]['total_floorsBengali'] = $_SESSION['new-officespace-info-error'][$rand]['total_floorsBengali'];
